...
from datetime import datetime #puts a timestamp on data we create
...
#think of these as admin (keeping track of products available to sell)
class User(db.Model, UserMixin): 
 ...
 #INSERT INTO User() Values()
    ...
    def set_id(self):
        return str(uuid.uuid4()) #creating a unique identification token
...
class Customer(db.Model):
    ...
    #this is how we tie a table to another one (not a column, but exablishing relationship)
    prodord = db.relationship('ProdOrder', backref = 'customer', lazy=True)
                                            #lazy = True means is we dont need the ProdOrder 
                                            # table to have a customer
...
products_schema = ProductSchema(many=True) #bringing baack all the products in our database & sending to front end
