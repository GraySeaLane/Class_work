...
#creating data usually gets a POST method
...
def create_order(cust_id):
    ...
    customer = Customer.query.filter(Customer.cust_id == cust_id).first()    #searching db for customer
...
def update_order(order_id):
    ...
    if prodorder.quantity > new_quantity: 
        ...
        order.decrement_ordertotal(prodorder.price) #total is less $ now

    elif prodorder.quantity < new_quantity:
        product.decrement_quantity(diff) #we are taking more quantity away
        order.increment_ordertotal(prodorder.price) # our total is more $ now
...
def delete_order(order_id):
    ...
    product.increment_quantity(prodorder.quantity) #getting back total quantity avail to sell
